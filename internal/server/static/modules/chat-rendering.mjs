...
import { codeBlockHTML, renderMarkdown } from "./chat-rendering-markdown.mjs";
import {
...
export function createChatRenderingController({
  ...
  function renderConversationRunNoticeHTML(run, status) {
    ...
    if (status === "interrupted") {
      // A user Stop (or an interrupt with no stored reason) already turns Send
      // into Continue. Repeating "this turn stopped" above the composer is
      // noise. A stored reason that is not a user Stop is usually the only
      // place the cause is visible, so those still get a line.
      const raw = String(run?.errorMessage || "").trim();
