...
export function normalizeAgentModelSettings(value = {}) {
  ...
  // Blank is meaningful: isolated plan review then follows the active
  // conversation model, so it must not fall back to defaultModel.
  const reviewModel = String(source.reviewModel || "").trim();
  ...
  return { defaultModel, summaryModel, reviewModel, safetyModel, defaultReasoningEffort, subagentModels, subagentModelPools };
}
...
export function createModelRoutingController(ctx) {
  ...
  // the global default. Isolated plan review uses inheritLabel for the active
  // conversation model. The unused safety-model field stays in the save payload
  // so editing other defaults does not wipe a leftover config.json value.
  ...
  function readAgentModelSettingsForm(form) {
    ...
    return normalizeAgentModelSettings({
      ...
      reviewModel: form?.elements?.reviewModel?.value || "",
      safetyModel: current.safetyModel || "",
  ...
  function validateAgentModelSettings(draft) {
    ...
    if (draft.reviewModel) checks.push([mt("routing.reviewModel"), draft.reviewModel]);
    for (const role of agentModelRoles) {
