...
const chatRendering = createChatRenderingController({
  ...
  retryLastRun: () => chatComposer.rerunLastUserMessage?.(),
...
backgroundTasks = createBackgroundTasksController({
  ...
  // Same profile the main transcript uses for user-message identity, so a
...
const agentStream = createAgentStreamController({
  api,
  webSocketURL,
  onEvent: (event) => handleAgentStreamEvent(event),
  onSnapshot: (snapshot, detail) => applyAgentLiveSnapshot(snapshot, detail),
...
const {
  applyAgentLiveSnapshot,
  handleAgentStreamEvent,
} = createAppMainStreamWiring({
  $,
  am,
  t,
  state,
...
async function persistAgentSettingsPass(snapshot) {
  ...
  try {
    ...
    // Model and thinking-effort options are rebuilt from the live catalog on
    // the change event. Re-entering the conversation used to wait on the
    // transcript round trip before those extra levels (xhigh/max) appeared.
    const selectedModel = model || $("modelSelect")?.value || "";
    renderModelOptions();
    refreshReasoningEffortControl({ modelValue: selectedModel });
    refreshFastModeControl({ modelValue: selectedModel });
    updatePermissionModeDisplay();
    updateWorkspaceMetaPills();
    if (state.agent?.id !== id) return;
