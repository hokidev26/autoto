...
`internal/peercontrol` owns the device identity, invitation envelope, pairing claim, session challenge, and bearer session used between two Autoto instances. `internal/server/remote_collaboration.go` exposes the owner-facing side under `/api/remote-collaboration` behind the sensitive local token guard, and `internal/server/remote_peer_api.go` exposes the inbound protocol under `/api/peer/v1`. `internal/server/static/modules/peer-collaboration-settings.mjs` is the settings page for invitations, pairings, project grants, and revocation. Project grants use the same picker pattern as collaborator memberships. Checking a project updates the draft in place instead of rebuilding the settings page. A host pairing may instead set `machineAccess`, which authorizes every current and later workspace project on that Autoto instance; the flag lives on the pairing row, per-agent grant rows stay empty, and inbound peer APIs recompute the live inventory on each request rather than trusting a client assertion. Archived conversations and standalone conversation-flow containers stay out of that inventory, matching the host project picker. Machine access is still capped by the pairing scopes and `permissionModeCap` (`readOnly` or `acceptEdits`); `bypassPermissions` stays rejected. Existing pairings stay per-project until the owner opts in. A stored pairing cap of `readOnly` is backfilled to `acceptEdits` when any grant already allowed edits, so turning on machine access cannot silently drop the ceiling. Revoked, expired, or rejected invitation rows and revoked or expired pairing rows can be deleted from that list after they have already been invalidated; open, claimed, and active rows cannot. Approving a new invitation from a peer that already has an active pairing revokes the previous row first so the unique fingerprint index does not surface a generic conflict. On the controller, `GET /api/remote-collaboration/peers/{id}/snapshot` also feeds the Remote collaboration rail category (`peer-collaboration-workspace.mjs`): shared agents are grouped by host in that sidebar and opened in a dedicated transcript that posts tasks, approvals, and runtime patches through the existing controller proxies. Message bodies go through the same Markdown pipeline as local chats (`chat-rendering-markdown.mjs`); HTML is escaped first, and only `http(s)` / `mailto` links survive. The snapshot includes the host model id and reasoning effort so the controller composer can show the same chips as a local chat. Snapshot messages also carry `parentToolUseId` so the controller can fold redacted `Tool … completed` dumps into the local step UI instead of rendering them as chat bubbles, and redacted `reasoningText` so those steps can still show the host's thinking; it still omits raw tool `inputJson`. Changing those chips (and the permission mode, clamped to the grant cap) requires `send_task` and `POST /api/peer/v1/agents/runtime`; an older host that lacks the route fails closed. Remote agent ids never enter local `/api/agents` routes or FK-constrained columns. The snapshot still omits the system prompt, workspace path, provider credentials, and execution-device details. Inbound peer sessions still require the in-memory sharing switch on the host: it starts off after every restart, and turning it off revokes live peer sessions even when the pairing row stays active. Owner-facing `/api/remote-collaboration` proxies map a dead peer credential to HTTP 409 rather than 401, so the local operator's Autoto account session is not treated as expired; inbound `/api/peer/v1` still returns 401 when the peer bearer fails. Every inbound peer request re-reads the pairing and compares its `credential_revision` and `grant_revision` against the values snapshotted when the bearer was issued, so revoking a pairing or re-authorizing its grants invalidates tokens already in the wild.

`internal/server/execution_transport.go` adds the device-facing half of remote execution on the same channel: `POST /api/peer/v1/execution/{heartbeat,claim,report}`, all requiring the `execute_tools` scope. A remote execution device is not a separate credential. `execution_devices.identity_fingerprint` is recorded at registration and compared inside `internal/db/execution_transport.go`, so only the pairing whose peer identity matches may act as that device; holding a valid bearer for a different pairing is not enough. A claim is restricted to the agents that pairing may currently reach: stored per-agent grants, or the live host inventory when `machineAccess` is on. The allow list is part of the claim query rather than a check on the delivered task, because a delivered payload cannot be recalled and the ledger has no leased-to-queued transition.
