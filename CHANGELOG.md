...
- Added an optional `timeoutSeconds` plugin manifest field (integer 1–600) that bounds every plugin operation — the whole enable/discover/health-check launch, and each tool call — instead of the fixed 20-second default. Absent or `0` keeps the default; like any manifest edit, adding or changing the field changes the manifest hash and requires update/reinstall before re-enabling, and older Autoto versions reject manifests containing it because unknown manifest fields are disallowed.
