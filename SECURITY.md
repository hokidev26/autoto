...
- `X-Autoto-Token` is the canonical browser API and WebSocket token header, and the only token header the server accepts. WebSocket clients may also provide the local token through the `autoto_local_token` cookie. The server no longer accepts query tokens; cookie or `X-Autoto-Token` only.
